...
/*   Updated: 2025/05/22 12:07:44 by rguigneb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec2	get_draw_width(t_entity *entity, int x)
{
	...
	draw_x.x = -entity->width / 2 + x;
	if (draw_x.x < 0)
		draw_x.x = 0;
	draw_x.y = entity->width / 2 + x;
...
{
	entity->width = abs((int)(SCREEN_H / transformed.y)) * entity->scale.x;
	entity->height = abs((int)(SCREEN_H / transformed.y)) * entity->scale.y;
...
void	render_entities(t_cub3d *cub3d)
{
	...
	while (i < cub3d->entity_count)
	{
		...
		cub3d->entities[i].y_offset = \
		(int)(-cub3d->entities[i].distance_from_floor / transformed.y);
		if (transformed.y > 0)
