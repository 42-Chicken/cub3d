...
/*   Updated: 2025/06/06 10:08:59 by rguigneb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		double half_height)
{
	...
	half_height = get_wall_height(data, ray) / 2;
	while (y < ray->floor_start - ray->skip)
	{
		distance_top = y + (half_height) - (SCREEN_H / 2);
		texture_loc.y = (wall_texture->height / 2.5) + (1LL * distance_top
				* wall_texture->height + half_height) / (half_height * 2);
