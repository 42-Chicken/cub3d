...
void	handle_house(t_cub3d *cub3d, t_texture *border, t_texture *house)
{
	...
	vec_to_spawn = (t_dvec2){cub3d->spawn_point.x + 0.5
		- cub3d->player.location.x, cub3d->spawn_point.y + 0.5
		- cub3d->player.location.y};
	rotated = (t_dvec2){vec_to_spawn.x * cub3d->player.cos_r - vec_to_spawn.y
		* cub3d->player.sin_r, vec_to_spawn.x * cub3d->player.sin_r
	...
	igmlx_simple_copy_to_dest(house,
								...
								(t_uvec2){
									cub3d->minimap.border_pos.x + border->width
										/ 2 - house->width / 2 + rotated.x * len
										* MINIMAP_TILE_SIZE,
									cub3d->minimap.border_pos.y + border->height
										/ 2 - house->height / 2 + rotated.y
										* len * MINIMAP_TILE_SIZE,
								});
}
